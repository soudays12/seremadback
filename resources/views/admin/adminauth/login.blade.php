...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-6">
  ...
  <main class="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
    <h1 class="text-2xl font-semibold text-gray-800 mb-6 text-center">Connexion 
    ...
    <form id="loginForm" action="{{ route('adminauth.authenticate') }}" method="POST" class="space-y-5" novalidate>
      @csrf
      <label class="block">
        ...
        <input type="email" name="email" required autofocus placeholder="prenom.nom@example.com"
          class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400"
      ...
      <label class="block relative">
        ...
        <input id="password" type="password" name="password" required minlength="6" placeholder="••••••••"
          class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 pr-12 focus:outline-none focus:ring-2 focus:ring-indigo-400"
